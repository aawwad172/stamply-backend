...
### 1. Identity Management (UUID v7)
- **Mandatory:** Use **UUID v7** for all new entity IDs to ensure time-ordered uniqueness.
- **Implementation:** Always use `GUID.GenerateVersion7()` for ID generation.
- **Restriction:** **DO NOT** use `GUID.NewGuid()`. Any usage of `GUID.NewGuid()` is considered a code smell and should be treated as a compilation warning/error where possible.

### 2. Repository Pattern
- **Generic Repository:** Always prefer the generic repository for standard CRUD operations (Create, Read, Update, Delete).
- **Custom Repositories:** Only create custom repositories if the required operation cannot be implemented efficiently or cleanly using the generic repository.

### 3. Error Handling & Results
- **Result Pattern:** Use a Result pattern (e.g., a `Result<T>` or `OneOf` approach) for methods where failure is a valid business outcome.
- **Exception Handling:** Rely on the centralized `ExceptionHandlerMiddleware` for truly unexpected errors. Do not use exceptions for flow control.

### 4. Minimal APIs & Versioning
- Use Minimal APIs for all new endpoints.
- Ensure every endpoint is versioned (e.g., `/api/v1/...`) to maintain backward compatibility.
